...
ll dp[5][MX+7];
int n, mark[5][MX+7], cas;
char str[MX+7];

ll rec(int pre, int pos){
    if( pos>=n )    return 1;
    ll &ret = dp[pre][pos];
    if( mark[pre][pos] == cas ) return ret;
    mark[pre][pos] = cas;
    ret = 0;
    if( str[pos] != 'W' ){
        if( id(str[pos]) == pre )   return ret = 0;
        ret = rec(id(str[pos]), pos+1);
        return ret;
    }
    ll res;
    for( int i = 1; i<4; i++ ){
        if( i == pre )  continue;
        ret = ( ret+rec(i, pos+1) )%mod;
    }
    return ret;
}
